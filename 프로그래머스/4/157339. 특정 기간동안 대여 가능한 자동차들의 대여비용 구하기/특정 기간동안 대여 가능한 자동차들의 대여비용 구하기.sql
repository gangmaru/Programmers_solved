...
SELECT C.CAR_ID, C.CAR_TYPE, (1-P.DISCOUNT_RATE/100)*C.DAILY_FEE*30 AS FEE
FROM TT C
...
JOIN CAR_RENTAL_COMPANY_DISCOUNT_PLAN P ON C.CAR_TYPE=P.CAR_TYPE
    AND P.DURATION_TYPE='30일 이상'
-- WHERE H.CAR_ID IS NULL 이유는 LEFT JOIN으로 대여한 날짜가 없어야 하기 때문에.
WHERE H.CAR_ID IS NULL 
    AND (1-P.DISCOUNT_RATE/100)*C.DAILY_FEE*30>=500000
    AND (1-P.DISCOUNT_RATE/100)*C.DAILY_FEE*30<2000000
